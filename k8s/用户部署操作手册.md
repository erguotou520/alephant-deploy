...
| 中间件密码 | PostgreSQL、ClickHouse、Valkey、MinIO 等连接使用 |
...
TCP:  ch-clickhouse.alephant-prod.svc.cluster.local:9000
```
...
部署前请把 `k8s/middlewares/valkey.values.yaml` 中的 `CHANGE_ME` 替换为真实密码。
...
README 说明：Qdrant 默认 3 节点集群。如果你的集群不是多节点生产环境，需要改成单节点配置：

- `replicaCount: 1`
- 关闭 `config.cluster.enabled`
...
README 特别说明：如果使用 `alicloud-disk-essd`，最小容量为 20Gi，低于此值 PVC 会创建失败。
...
项目安装脚本会生成随机密钥、创建 Secret/ConfigMap，并初始化数据库。

执行前确认：

- 中间件已经全部 Running
- PostgreSQL、ClickHouse、Valkey、MinIO 等密码与 values 文件一致
...
helm upgrade --install alephant-prod weconomy/common \
  ...
  --create-namespace \
  -f k8s/values.yaml \
  --set saasService.env.PRIVATE_WORKSPACE_OWNER_EMAILS.value="$PRIVATE_WORKSPACE_OWNER_EMAILS"
```
...
| `openmodels.link` | `/v1/policy` | `alephant-policy-service` |
...
```bash
kubectl port-forward -n "$NAMESPACE" svc/alephant-app 18000:80
```
...
如果使用 `alicloud-disk-essd`，注意 README 中提到的最小容量 20Gi。
...
- Helm 部署时设置了 `PRIVATE_WORKSPACE_OWNER_EMAILS`
...
- ClickHouse 地址：`http://ch-clickhouse:8123`
...
如果数据库不是全新环境，可能出现表已存在、schema 已存在等错误。不要反复导入 SQL。

先检查表是否已经存在：

```bash
kubectl exec -n "$NAMESPACE" <POSTGRES_POD_NAME> -- \
  psql -U alephant -d alephant -tAc "SELECT count(*) FROM information_schema.tables WHERE table_schema='public';"
```
